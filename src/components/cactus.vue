...
<script>
	export default {
		...
		created() {
			if(this.$parent.$parent.visible) setTimeout(this.init,1000);
		},
		methods: {
			init(){
			...
			add() {
				...
				if (this.cactus.length >= cTotal) return;
				this.createCactus();
				let lagAmend = 1;
				if (difficulty == "easy") lagAmend = 1.2;
				if (difficulty == "hard") lagAmend = 0.8;
				if (difficulty == "hell") lagAmend = 0.5;
				this.lag = this.createRandom2(4, 8) * lagAmend;
			...
			createRandom2(start, end) { //生成随机数-小数
				let abs = Math.abs(start - end);
				return (start + Math.random() * abs).toFixed(2);
			},
		}
	}
</script>
...
<style lang="less" scoped>
	...
	.cactus span {
		...
		&.c1 {
			background-image: url(../assets/cactus/c1.svg)
		}

		&.c2 {
			background-image: url(../assets/cactus/c2.svg)
		}

		&.c3 {
			background-image: url(../assets/cactus/c3.svg)
		}

		&.c4 {
			background-image: url(../assets/cactus/c4.svg)
		}

		&.c5 {
			background-image: url(../assets/cactus/c5.svg)
		}
	}
